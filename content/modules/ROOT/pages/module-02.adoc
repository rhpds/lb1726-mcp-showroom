...
[NOTE]
====
*Prerequisite:* Complete xref:module-01.adoc[Module 1: Lab Setup] before starting this module.
====
...
----
============================================================
🚀 Pipeline Failure Agent - Starting up
============================================================
...
🔌 Initializing MCP connections (2 servers)...
...
✅ Startup complete - 129 tools available
============================================================
...
. We prepared a branch in the Gitea repository with the name *broken*. Feel free to show that branch in your Gitea repository and point out that the only difference in that directory is that the file `requirements.txt` has accidentally been deleted and is therefore missing.
...
----
What pods are running in namespace agent-{user}
----
...
[source,role="execute",subs=attributes]
----
What pods in namespace agent-{user} are in Error state?
----
...
.. You could try to delete pods in namespace `agent-{user}`. Why can/can't you delete pods in the `agent-{user}` namespace?
.. You could try to delete pods in namespace `mcp-openshift-{user}`?  Why can/can't you delete pods in the `mcp-openshift-{user}` namespace?
...
. Open the *agent* directory. Notice that this is a (relatively simple) Pyhon app. Feel free to show the source code and focus on the prompts that the agent is sending to the LLM.
