...
In Module 1, you experienced MCP in action — an autonomous agent diagnosing pipeline failures and creating issues, plus interactive infrastructure queries through LibreChat. The MCP servers in that demo were pre-configured for ease of use, with all tools enabled and minimal security constraints.
...
. *Edit the file* — Click the edit button (pencil icon) to modify the file directly in the browser. Alternatively, you could clone the repo and use your favorite editor.
...
== Adding a Simple MCP Server: Time Service

=== Extending AI Capabilities

One of MCP's strengths is the growing ecosystem of MCP servers. Adding new capabilities to your AI agents is often as simple as deploying another MCP server. In this section, you'll deploy a simple Time MCP server to demonstrate the process.

The Time MCP server provides tools for:
...
=== Business Value of Standardized Time Handling

Even a "simple" capability like time handling has business value:

* *Distributed Teams*: AI agents can provide timezone-aware responses ("Schedule this for 9 AM in Tokyo, which is 5 PM Pacific")
* *Audit Consistency*: All AI-generated timestamps use consistent formatting
* *Compliance*: Financial and regulatory systems often require specific timestamp formats

=== Deploying the Time MCP Server

. In Gitea, navigate to `{user}/mcp/helm` and create a new directory `mcp-time`.

. Create a file `mcp-time/Chart.yaml`:
+
[source,yaml,role=execute]
----
apiVersion: v2
...
. Create a file `mcp-time/values.yaml`:
+
[source,yaml,role=execute]
----
namespace: mcp-time-{user}
----

. Create `mcp-time/templates/mcpserver.yaml`:
+
[source,yaml,role=execute]
----
---
...
metadata:
  ...
  namespace: {{ .Values.namespace }}
spec:
  image: ghcr.io/modelcontextprotocol/time:latest
  transport: stdio
  telemetry:
    prometheus:
      enabled: true
...
. Add the new Helm chart to your ArgoCD Application or wait for GitOps to sync.

. Once deployed, add the Time MCP server to LibreChat and test it:
+
[source,role=execute]
----
