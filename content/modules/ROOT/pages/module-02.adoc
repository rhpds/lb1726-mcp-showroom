...
====
. Commit the file
. Check that the new MCP Server has been rolled out (I had to sync argo... need to make the check much shorter)
...
====
. Commit and sync.
. Hrm. Can't be seen by regular users. As Admin you could go in the OpenShift Console to Observe -> Metrics and query for `toolhive_mcp_active_connections` or `toolhive_mcp_requests_total`.
