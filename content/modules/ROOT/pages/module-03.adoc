...
[NOTE]
====
*Prerequisites:* Complete xref:module-01.adoc[Module 1: Lab Setup] before starting this module.
====
...
[TIP]
====
Add the `toolConfigRef` block in the `spec:` section of your MCPServer, at the same indentation level as `image` and `transport`.
====
+
[source,yaml,role=execute]
----
...
.. In Librechat try to list the pods in the `agent-{user}` namespace again:
+
[source,role=execute,subs=attributes]
----
List the running pods in namespace agent-{user}.
----
+
You see that the two running pods are returned.

.. If you did the optional parts in the previous lab you saw that were able to delete any pod in the `agent-{user}` namespace.
+
Try to delete one of the pods in that namespace again (replace the pod name with the name of the agent pod that the previous question returned):
+
[source,sh,role=execute,subs=attributes]
----
Delete pod agent-xxxxxxx in namespace agent-{user}
----
+
...
----
Assistant sent this info to openshift
{
  "name": "agent-6d8777b969-v2z9p",
...
[TIP]
====
Add this `telemetry:` block under `spec:` at the same indentation level as other spec fields.
====
+
[source,yaml,role=execute]
----
...
[source,sh,role=execute,subs=attributes]
----
curl -s https://mcp-openshift-mcp-openshift-{user}.{openshift_cluster_ingress_domain}/metrics
----
+
[TIP]
====
The `-s` flags: `-s` suppresses progress output.
====
+
...
. Let's create the ServiceMonitor. Create a new file `servicemonitor.yaml` in the OpenShift MCP server's `templates` directory:
+
[source,yaml,role=execute]
----
---
...
. In the OpenShift Console navigate to *Observe* -> *Metrics* and query for `toolhive_mcp_active_connections` or `toolhive_mcp_requests_total`. If you see "No datapoints found" the service monitor hasn't synced yet. Try again a few seconds later.
...
. Commit the file.
. Wait until GitOps has synced and the Fetch MCP server has been deployed — you can check in namespace `mcp-gitea-{user}` that two new pods are running. The MCP server in the `fetch-0` pod and the ToolHive proxy whose name also starts with `fetch-`.
...
. Commit the file.
. Wait until GitOps has synced and the Yardstick MCP server has been deployed — you can check in namespace `mcp-gitea-{user}` that two new pods with names starting with `yardstick-` are running (you may need to scroll out in the *Topology* view to see all 6 pods).
...
[NOTE]
====
In early January or 2026 it was decided to go a different direction for the Red Hat official MCP server / registry tooling. This means that Red Hat will not use ToolHive in the supported solution.

However the new direction means that no real code is available to demonstrate where we will be going. Therefore we decided to keep the current lab - since most concepts will stay exactly the same.

MCP Registry is a standard API - so whatever you learned in this lab will still be useful regardless of how the final solution will shape out to be.
====
