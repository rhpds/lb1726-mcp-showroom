...
. Commit the file.
. Wait until GitOps has synced and the Yardstick MCP server has been deployed — you can check in namespace `mcp-gitea-{user}` that two new pods with names atarting with `yardstick-` are running (you may need to scroll out in the *Topology* view to see all 6 pods).
