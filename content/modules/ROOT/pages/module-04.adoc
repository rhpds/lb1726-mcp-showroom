...
[NOTE]
====
*Prerequisites:* Complete xref:module-01.adoc[Module 1], and xref:module-03.adoc[Module 3] before starting this module.
====
...
+
Create a PostgreSQL database by adding file `psql.yaml` under the `templates` directory of the Gitea MCP Server Helm chart:
+
...
[start=2]
. Create an empty `ConfigMap` to hold the MCP Registry configuration. Add file `configmap-mcp-registry-config.yaml` to the `templates` directory:
+
[source,yaml,role=execute,subs=attributes]
----
---
...
Add file `secret-mcp-registry-db-password.yaml` in the `templates` directory:
+
[source,yaml,role=execute,subs=attributes]
----
---
...
----
curl -s https://mcp-registry-mcp-gitea-{user}.{openshift_cluster_ingress_domain}/registry/mcp-registry/v0.1/servers | jq
----
+
...
[start=10]
. Query the registry to see the auto-registered servers (note that you are using the `default` registry, not our configured `mcp-registry`):
+
[source,sh,role=execute,subs=attributes]
----
curl -s https://mcp-registry-mcp-gitea-{user}.{openshift_cluster_ingress_domain}/registry/default/v0.1/servers | jq
----
+
...
. In Gitea navigate back to the `templates` directory and edit the ConfigMap `configmap-mcp-registry-config.yaml` (you can simply overwrite the existing definition with this one) to add the *Fetch* MCP Server to your registry.
+
[source,yaml]
----
---
  ...
  registry.json: |
    {
      "$schema": "https://raw.githubusercontent.com/stacklok/toolhive/main/pkg/registry/data/schema.json",
      "version": "1.0.0",
      "last_updated": "2026-02-06T13:00:00Z",
      "servers": {
        "fetch": {
          "name": "fetch",
          "description": "A Model Context Protocol server that provides web fetching and content retrieval capabilities with streamable HTTP transport.",
...
----
+
You will see that now we have 1 server available - if you query the `default` registry again you'll see that we still have 2 servers registered there.
