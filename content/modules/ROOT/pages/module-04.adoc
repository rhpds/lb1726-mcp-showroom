...
Now let's enable enforcement on your registry. This requires adding a single field to the MCPRegistry resource.

[start=8]
. Update the MCPRegistry to enable enforcement. In Gitea, edit the `mcpregistry.yaml` file in your Helm chart templates and add `enforceServers: true`:
+
[source,yaml,role=execute,subs=attributes]
----
---
...
With enforcement enabled, let's verify it works by attempting to deploy an unapproved MCP server.

[start=11]
. Create a test MCPServer resource that references an image NOT in your registry. Add file `mcpserver-unapproved.yaml` to your templates:
+
[source,yaml,role=execute,subs=attributes]
----
---
...
metadata:
  ...
  namespace: {{ .Values.namespace }}
spec:
  image: ghcr.io/random-org/random-mcp-server:latest  # Not in registry
  transport: sse
  port: 8080
----

. Commit and push the change. Watch ArgoCD attempt to sync.
. Observe the rejection. The MCPServer resource will fail to create with an error similar to:
+
----
Error: admission webhook "mcpserver.toolhive.stacklok.dev" denied the request:
image "ghcr.io/random-org/random-mcp-server:latest" not found in any
enforcing registry in namespace "mcp-gitea-{user}"
----
+
This error confirms that:
+
* The enforcement webhook intercepted the MCPServer creation
* The image was checked against the registry catalog
* The deployment was blocked because the image isn't approved

. Clean up by removing or commenting out the `mcpserver-unapproved.yaml` file.
...
== Deploying Registry-Approved Servers

Now let's deploy a server that IS in the registry to see the successful path.

[start=15]
. First, verify the Kubernetes MCP server is in your registry:
+
[source,sh,role=execute,subs=attributes]
----
curl -s https://mcp-registry-mcp-gitea-{user}.{openshift_cluster_ingress_domain}/registry/v0.1/servers | \
  jq '.servers[] | select(.server.name | contains("kubernetes")) | .server.packages[].identifier'
----
+
This returns the approved image: `quay.io/containers/kubernetes_mcp_server:latest-linux-amd64`

. Create an MCPServer using this approved image. The server will deploy successfully because the image exists in the enforcing registry.
...
* *Deployment enforcement*: Validation that prevents unapproved servers from running
