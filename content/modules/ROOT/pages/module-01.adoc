...
. In the panel on the right switch to the *Gitea* tab. Log in using user *{gitea_user}* with password *{gitea_password}*.
...
. Switch to *Workloads -> Pods* and open the *agent-xxxxx* pod (your specific pod name will be different).
. Show the pod logs and point out what the agent did. The interesting bit starts with *📥 Received failure report*. In *Iteration 1* the agent gets the pod logs from OpenShift while in *Iteration 2* the agent opens a new issue in Gitea.
