...
*Module 1: Sovereign SRE Agent Demo* — Experience an end-to-end scenario where an AI agent automatically diagnoses a CI/CD pipeline failure. You'll see how the agent uses the OpenShift MCP Server to retrieve logs and the Gitea MCP Server to create an issue — all without any custom integration code. You'll also use LibreChat to interactively query your infrastructure using natural language.

*Module 2: MCP Server Administration* — Learn how to configure and manage MCP servers in production. This includes tool filtering (restricting which capabilities are exposed), telemetry and observability, and advanced topics like MCP registries.
...
* A predeployed *Gitea MCP Server*: Provides 57 tools for Git repository operations (issue creation, pull requests, file management)
...
*MCP Registry*:: A catalog of approved MCP servers within an organization. Developers can discover available servers, and administrators can control which servers are sanctioned for use.
...
Module 2 of this lab demonstrates several of these enterprise hardening techniques hands-on:
